...
static int grow_hashtable(HT *h, size_t oldsize)
{
    ...
    for (oldi = 0; oldi < h->wpd.neighborhood_len; oldi++) {
        ...
        for (oldj = 0; oldj < NEIGHBORHOOD_LEN; oldj++) {
            ...
            if (oldv == NULL)
                continue;
            hash = h->config.ht_hash_fn(oldv->value.key.keybuf,
                                        oldv->value.key.keysize);
            newi = hash & newmd->neighborhood_mask;
            rehashed = 0;
                                                    ...
                                                    uintptr_t *type)
{
    ...
    size_t nvsize = sizeof(*tmp) + key->keysize;
...
HT_VALUE *ossl_ht_get(HT *h, HT_KEY *key)
{
    ...
    do {
        ...
        for (j = 0; j < NEIGHBORHOOD_LEN; j++) {
            ...
            if (match_key(&ival->value.key, key))
